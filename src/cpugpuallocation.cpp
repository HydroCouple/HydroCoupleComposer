...
CPUGPUAllocation *CPUGPUAllocation::readCPUGPUAllocation(QXmlStreamReader &xmlReader, GModelComponent *parent, QList<QString> &errorMessages)
{
  ...
  if(!xmlReader.name().compare("ComputeResourceAllocation",Qt::CaseInsensitive) && !xmlReader.hasError() && xmlReader.tokenType() == QXmlStreamReader::StartElement )
  {
    int tempValue = 0;
    ...
    while(!(xmlReader.isEndElement() && !xmlReader.name().compare("ComputeResourceAllocation", Qt::CaseInsensitive)) && !xmlReader.hasError())
    {
      xmlReader.readNext();
    }
  }
